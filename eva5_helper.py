import torch
from .models.model import compute_losses, l1LossMask, l2LossMask, unmoldDetections, calcXYZModule, invertDepth, warpModuleDepth, l2NormLossMask
...
class PlaneRCNNTrainer:
    ...
    def train_step(self, batch, output):
        ...
        images, image_metas, rpn_match, rpn_bbox, gt_class_ids, gt_boxes, gt_masks, gt_parameters, gt_depth, extrinsics, gt_plane, gt_segmentation, plane_indices, camera = batch
        ...
        rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, target_parameters, mrcnn_parameters, detections, detection_masks, detection_gt_parameters, detection_gt_masks, rpn_rois, roi_features, roi_indices, feature_map, depth_np_pred = output
        rpn_class_loss, rpn_bbox_loss, mrcnn_class_loss, mrcnn_bbox_loss, mrcnn_mask_loss, mrcnn_parameter_loss = compute_losses(self.config, rpn_match, rpn_bbox, rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, target_parameters, mrcnn_parameters)
        ...
        if len(detections) > 0:
            detections, detection_masks = unmoldDetections(self.config, camera, detections, detection_masks, depth_np_pred, normal_np_pred, debug=False)
            ...
            XYZ_pred = torch.zeros((3, self.config.IMAGE_MAX_DIM, self.config.IMAGE_MAX_DIM)).cuda()
            detection_mask = torch.zeros((1, self.config.IMAGE_MAX_DIM, self.config.IMAGE_MAX_DIM)).cuda()
            plane_XYZ = torch.zeros((1, 3, self.config.IMAGE_MAX_DIM, self.config.IMAGE_MAX_DIM)).cuda()                        
            pass
        ...
        if (len(detection_pair[0]['detection']) > 0 and len(detection_pair[0]['detection']) < 30) and 'refine' in self.options.suffix:
            ...
            image = (input_dict['image'] + self.config.MEAN_PIXEL_TENSOR.view((-1, 1, 1))) / 255.0 - 0.5
            image_2 = (input_pair[1 - c]['image'] + self.config.MEAN_PIXEL_TENSOR.view((-1, 1, 1))) / 255.0 - 0.5
            ...
            if 'refine_after' in self.options.suffix:
                ...
                XYZ_pred, detection_mask, plane_XYZ = calcXYZModule(self.config, camera, detections, detection_masks, detection_dict['depth_np'], return_individual=True)
        ...
        for c in range(1, 2):
            ...
            warped_info, valid_mask = warpModuleDepth(self.config, camera, input_pair[c]['depth'][0], neighbor_info, input_pair[c]['extrinsics'][0], input_pair[1 - c]['extrinsics'][0], width=config.IMAGE_MAX_DIM, height=config.IMAGE_MIN_DIM)
            ...
            if 'warping2' in self.options.suffix or 'warping3' in self.options.suffix:
                warped_image_loss = l2NormLossMask(warped_images, input_pair[c]['image'], dim=1, valid_mask=valid_mask)

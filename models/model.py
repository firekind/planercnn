...
class FPNWithExternalExtractor(nn.Module):
    def __init__(self, out_channels, bilinear_upsampling=False):
        super(FPNWithExternalExtractor, self).__init__()
        self.out_channels = out_channels
        self.bilinear_upsampling = bilinear_upsampling
    ...
    def forward(self, extractor_stages):
        c2_out = extractor_stages[0]
        c3_out = extractor_stages[1]
        c4_out = extractor_stages[2]
        c5_out = extractor_stages[3]
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        if not config.EXTERNAL_EXTRACTOR:
            ...
            self.fpn = FPNWithExternalExtractor(out_channels=256, bilinear_upsampling=self.config.BILINEAR_UPSAMPLING)
